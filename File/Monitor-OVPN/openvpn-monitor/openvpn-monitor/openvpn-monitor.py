...
class OpenvpnHtmlPrinter(object):
    ...
    def print_html_header(self):
        ...
        output('<title>KingKongVPN {0!s} Monitor </title>'.format(self.site))
        output('<meta content="https://t.me/xiihaiqal" name="author" />')
        output('<meta http-equiv="refresh" content="10" />')

        # css
        output('<link rel="shortcut icon" type="image/x-icon" href="https://raw.githubusercontent.com/xiihaiqal/AutoScriptVPS/master/Snapshots/openvpn.png">')
        ...
        output('KingKongVPN {0!s} Monitor </a>'.format(self.site))

        output('</div><div class="collapse navbar-collapse" id="myNavbar">')
        output('<ul class="nav navbar-nav"><li class="dropdown">')
        output('<a class="dropdown-toggle" data-toggle="dropdown" href="#">Pipe')
        ...
        if self.maps:
            ...
            output('<li><a href="https://paypal.me/kingkongvpn">Paypal Donate</a></li>')    
            output('<li><a href="https://t.me/xiihaiqal">Contact</a></li>')                
    ...
    def print_session_table_headers(vpn_mode, show_disconnect):
        server_headers = ['Users',
                          'Location',
                          'Download',
                          'Time Online', 'Upload', 'Connected Since', 'Last Ping']
        if show_disconnect:
    ...
    def print_vpn(self, vpn_id, vpn):
        ...
        output('<thead><tr><th> </th><th>Status</th><th>Total Download</th><th>Total Upload</th>')
        output('<th>Pingable</th><th>Clients</th>')
...
#        output('<td>{0!s}</td>'.format(session['local_ip']))
        ...
        if 'location' in session and session['location'] is not None:
            if session['location'] == 'RFC1918':
                ...
                if 'region' in session and session['region'] is not None:
                    ...
                    full_location = '{1!s}'.format(region, full_location)
                if 'city' in session and session['city'] is not None:
                    city = session['city']
                    full_location = '{1!s}'.format(city, full_location)
            ...
            if 'sessions' in vpn:
                ...
                for skey, session in list(vpn['sessions'].items()):
                    ...
                    if 'longitude' in session and 'latitude' in session:
                        ...
                        output('client_popup.setContent("{0!s} - CONNECTED");'.format(
    ...
    def print_html_footer(self):
        ...
        output('Page automatically reloads every 10 Seconds.')
