...
#	Version: 1.0.26                            
#	youtube:https://www.youtube.com/c/ComeBey
#       Twitter:https://twitter.com/WangTao_Im
#       blog:https://www.rootfw.com
#	
#==============================================================================================
...
check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error} The current account is not ROOT (or does not have ROOT permissions) and cannot continue to operate. Please use ${Green_background_prefix} sudo su ${Font_color_suffix} to obtain temporary ROOT permissions (you will be prompted to enter the password of the current account after execution)." && exit 1
}
check_sys(){
...
check_crontab(){
	[[ ! -e "/usr/bin/crontab" ]] && echo -e "${Error} Crontab is missing, please try to install CentOS manually: yum install crond -y, Debian/Ubuntu: apt-get install cron -y !" && exit 1
}
SSR_installation_status(){
	[[ ! -e ${ssr_folder} ]] && echo -e "${Error} No ShadowsocksR folder is found, please check !" && exit 1
}
...
ssr_link_qr(){
	...
	SSRQRcode="https://www.codigos-qr.com/qr/php/qr_img.php?d=${SSRurl}"
...
The [_compatible] behind the protocol and obfuscation refers to compatibility with the original protocol/obfuscation。"
	echo && echo "==================================================="
}
...
Set_config_port(){
	...
	read -e -p "(default: 5210):" ssr_port
...
Set_config_method(){
 ...
 ${Tip} If you use the auth_chain_* series of protocols, it is recommended to select none for the encryption method (this series of protocols comes with RC4 encryption).
 ...
 ${Tip} The salsa20/chacha20-* series of encryption methods require additional installation and dependency on libsodium, otherwise ShadowsocksR cannot be started!" && echo
...
Set_config_protocol(){
 ...
 ${Tip} If you use the auth_chain_* series of protocols, it is recommended to select none for the encryption method (this series of protocols comes with RC4 encryption), and you can confuse it at will" && echo
	...
	if [[ ${ssr_protocol} != "origin" ]]; then
		if [[ ${ssr_protocol} == "auth_sha1_v4" ]]; then
			read -e -p "Is the protocol plug-in compatible with the original version (_compatible)? [Y/n]" ssr_protocol_yn
...
Set_config_obfs(){
 ...
 In addition, if you choose tls1.2_ticket_auth, then the client can choose tls1.2_ticket_fastauth, so that it can be disguised without increasing the delay!
 If you are building in popular areas such as Japan, the United States, etc., you may have a lower chance of being confused by the wall if you choose plain!" && echo
	...
	if [[ ${ssr_obfs} != "plain" ]]; then
			read -e -p "Is the obfuscation plug-in compatible with the original version (_compatible)? [Y/n]" ssr_obfs_yn
...
Set_config_protocol_param(){
	...
	if [[ $? == 0 ]]; then
		if [[ ${ssr_protocol_param} -ge 1 ]] && [[ ${ssr_protocol_param} -le 9999 ]]; then
			...
			echo -e "${Error} Device limit(1-9999)"
		fi
	else
		echo -e "${Error} Device limit(1-9999)"
	fi
...
Set_config_speed_limit_per_user(){
	...
	echo -e "${Tip} Total port speed limit: The total speed of each port is the upper limit of the speed limit, and the overall speed of a single port is limited."
...
Set_config_forbid(){
	echo "Please enter the user to be set up"
	echo -e "${Tip} Prohibited ports: For example, if you are not allowed to access port 25, users will not be able to access mail port 25 through the SSR proxy. If 80,443 is prohibited, users will not be able to access http/https websites normally.
Block a single port format: 25
Block multiple port formats: 23,465
Block port segment format: 233-266
Block multiple format ports: 25,465,233-666 (Without colon:)"
	read -e -p "(default : Empty does not prohibit access to any port):" ssr_forbid
...
Set_config_enable(){
	...
	do
		...
		if [[ "${ssr_port}" == "${port_jq}" ]]; then
			...
			[[ "${enable}" == "null" ]] && echo -e "${Error} Failed to obtain the disabled status of the current port [${ssr_port}] !" && exit 1
			ssr_port_num=$(cat "${config_user_mudb_file}"|grep -n '"port": '${ssr_port}','|awk -F ":" '{print $1}')
			echo -e "ssr_port_num=${ssr_port_num}"
			[[ "${ssr_port_num}" == "null" ]] && echo -e "${Error} Failed to obtain the disabled status of the current port [${ssr_port}] !" && exit 1
	...
	if [[ "${enable}" == "1" ]]; then
		echo -e "port [${ssr_port}] The account status is: ${Green_font_prefix} enable ${Font_color_suffix} , whether to switch to ${Red_font_prefix} disable ${Font_color_suffix} ?[Y/n]"
		...
		echo -e "port [${ssr_port}] The account status of is:${Green_font_prefix}Disable${Font_color_suffix} , Whether to switch to ${Red_font_prefix}Enable${Font_color_suffix} ?[Y/n]"
		read -e -p "(default: Y):" ssr_enable_yn
		[[ -z "${ssr_enable_yn}" ]] && ssr_enable_yn="y"
		if [[ "${ssr_enable_yn}" == [Yy] ]]; then
			ssr_enable="1"
...
Set_user_api_server_pub_addr(){
	...
	read -e -p "(default : automatically detect external network IP):" ssr_server_pub_addr
...
Modify_config_method(){
	...
	if [[ -z "${match_edit}" ]]; then
		...
		echo -e "${Info} The port user encryption method is successfully modified ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: it may take about ten seconds to apply the latest configuration)"
	fi
}
...
Modify_config_obfs(){
	...
	if [[ -z "${match_edit}" ]]; then
		echo -e "${Error} User obfs modification failed ${Green_font_prefix}[port: ${ssr_port}]${Font_color_suffix} " && exit 1
	else
		echo -e "${Info} User obfs modified successfully ${Green_font_prefix}[port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds to apply the latest configuration)"
	fi
}
Modify_config_protocol_param(){
	match_edit=$(python mujson_mgr.py -e -p "${ssr_port}" -G "${ssr_protocol_param}"|grep -w "edit user ")
	if [[ -z "${match_edit}" ]]; then
		echo -e "${Error} User protocol parameter (device number limit) modification failed ${Green_font_prefix}[port: ${ssr_port}]${Font_color_suffix} " && exit 1
	else
...
Modify_config_speed_limit_per_user(){
	...
	if [[ -z "${match_edit}" ]]; then
		echo -e "${Error} User port total rate limit modification failed ${Green_font_prefix}[port: ${ssr_port}]${Font_color_suffix} " && exit 1
	else
		echo -e "${Info} The total rate limit of the user port is successfully modified ${Green_font_prefix}[port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds to apply the latest configuration)"
	fi
}
...
Modify_config_transfer(){
	...
	if [[ -z "${match_edit}" ]]; then
		...
		echo -e "${Info} The total user traffic is modified successfully ${Green_font_prefix}[port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds to apply the latest configuration)"
	fi
}
...
Download_SSR(){
	...
	echo -e "${Info} ShadowsocksR server apiconfig.py copy failed !"
}
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
...
Installation_dependency(){
	...
	[[ ! -e "/usr/bin/unzip" ]] && echo -e "${Error} Rely on unzip (unzip compressed package) installation failed, most of the problem is the source of the package, please check !" && exit 1
	Check_python
...
Install_SSR(){
	check_root
	[[ -e ${ssr_folder} ]] && echo -e "${Error} The ShadowsocksR folder already exists, please check (if the installation fails or there is an old version, please uninstall first) !" && exit 1
	echo -e "${Info} Start to set up ShadowsocksR account configuration..."
	Set_user_api_server_pub_addr
	Set_config_all
...
Update_SSR(){
	SSR_installation_status
	echo -e "This function is temporarily disabled due to the suspension of updating the ShadowsocksR server by Powa."
	#cd ${ssr_folder}
	#git pull
	#Restart_SSR
}
Uninstall_SSR(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		echo && echo " ShadowsocksR uninstallation complete !" && echo
	else
...
Install_Libsodium(){
	if [[ -e ${Libsodiumr_file} ]]; then
		echo -e "${Error} Libsodium is already installed, should you overwrite the installation (update)? [y/N]"
	...
	echo && echo -e "${Info} libsodium is installed successfully!" && echo
}
...
debian_View_user_connection_info(){
	...
	echo -e "Total users: ${Green_background_prefix} "${user_total}" ${Font_color_suffix} Total link IP: ${Green_background_prefix} "${IP_total}" ${Font_color_suffix} "
...
View_user_connection_info(){
 ...
 ${Green_font_prefix}2.${Font_color_suffix} Display IP+IP attribution format" && echo
	...
	if [[ ${ssr_connection_info} == "1" ]]; then
		...
		echo -e "${Tip} Check the IP attribution (ipip.net), if there are more IPs, the time may be longer..."
...
————— Modify user configuration —————
 ...
 ${Green_font_prefix}5.${Font_color_suffix}  Modify the protocol plugin
...
Clear_transfer(){
	SSR_installation_status
	echo && echo -e "What are you going to do？
 ${Green_font_prefix}1.${Font_color_suffix}  Cleared data used by a single user
 ${Green_font_prefix}2.${Font_color_suffix}  Clear all users' used data (irrecoverable)
 ${Green_font_prefix}3.${Font_color_suffix}  Start time all user traffic is cleared
 ${Green_font_prefix}4.${Font_color_suffix}  Stop all user traffic is cleared regularly
 ${Green_font_prefix}5.${Font_color_suffix}  Modify all user traffic is cleared regularly" && echo
	read -e -p "(default: cancel):" ssr_modify
	[[ -z "${ssr_modify}" ]] && echo "Cancelled..." && exit 1
	if [[ ${ssr_modify} == "1" ]]; then
		Clear_transfer_one
...
Clear_transfer_one(){
	...
	while true
	do
		echo -e "Please enter the user port to clear the used data"
...
Clear_transfer_all(){
	...
	do
		...
		if [[ -z "${match_clear}" ]]; then
			echo -e "${Error} Failed to clear the user's used data ${Green_font_prefix}[port: ${user_port}]${Font_color_suffix} "
		else
			echo -e "${Info} The user has used the traffic to zero successfully ${Green_font_prefix}[port: ${user_port}]${Font_color_suffix} "
		fi
	done
...
Clear_transfer_all_cron_start(){
	...
	if [[ -z ${cron_config} ]]; then
		...
		echo -e "${Info} All user traffic is cleared and started successfully !"
	fi
}
Clear_transfer_all_cron_stop(){
	crontab -l > "$file/crontab.bak"
	...
	if [[ ! -z ${cron_config} ]]; then
		...
		echo -e "${Info} All user traffic is cleared regularly and stopped successfully !"
	fi
}
Clear_transfer_all_cron_modify(){
	Set_crontab
...
Set_crontab(){
		echo -e "Please enter the interval for clearing the flow
 === Format description ===
 * * * * * Corresponding to minute, hour, day, month, week
 ${Green_font_prefix} 0 2 1 * * ${Font_color_suffix} On behalf of every month at 2 o'clock on the 1st, clear the used traffic
 ${Green_font_prefix} 0 2 15 * * ${Font_color_suffix} On behalf of the 15th of each month at 02:00 pm Clear the used traffic
 ${Green_font_prefix} 0 2 */7 * * ${Font_color_suffix} It means that the used traffic will be cleared at 2 o'clock every 7 days
 ${Green_font_prefix} 0 2 * * 0 ${Font_color_suffix} On behalf of every Sunday (7) clear the used flow
 ${Green_font_prefix} 0 2 * * 3 ${Font_color_suffix} On behalf of every Wednesday (3) clear the used flow" && echo
	read -e -p "(default: 0 2 1 * * At 2 o'clock on the 1st of every month):" Crontab_time
...
View_Log(){
	...
	echo && echo -e "${Tip} press ${Red_font_prefix}Ctrl+C${Font_color_suffix} Stop viewing the log "&& echo -e" If you need to view the complete log content, please use the ${Red_font_prefix}cat ${ssr_log_file}${Font_color_suffix} command."&& echo
	tail -f ${ssr_log_file}
}
# Server Speeder
Configure_Server_Speeder(){
	echo && echo -e "What are you going to do？
 ${Green_font_prefix}1.${Font_color_suffix} Installation Server Speeder
 ${Green_font_prefix}2.${Font_color_suffix} Uninstall Server Speeder
————————
 ${Green_font_prefix}3.${Font_color_suffix} Start Server Speeder
 ${Green_font_prefix}4.${Font_color_suffix} Stop Server Speeder
 ${Green_font_prefix}5.${Font_color_suffix} Restart Server Speeder
 ${Green_font_prefix}6.${Font_color_suffix} View Server Speeder Status
 
 Note: Server Speeder and LotServer cannot be installed/started at the same time！" && echo
...
Install_LotServer(){
	...
	wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/0oVicero0/serverSpeeser_Install/master/appex.sh"
...
Uninstall_LotServer(){
	...
	[[ -z ${unyn} ]] && echo && echo "Cancelled..." && exit 1
	if [[ ${unyn} == [Yy] ]]; then
		wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/0oVicero0/serverSpeeser_Install/master/appex.sh" && bash /tmp/appex.sh 'uninstall'
		echo && echo "LotServer uninstallation complete !" && echo
	fi
}
# BBR
Configure_BBR(){
...
1. Install and open BBR, need to replace the kernel, there are risks such as replacement failure (cannot boot after restart)
...
3. During the process of kernel replacement, Debian will prompt [Whether to terminate uninstalling the kernel], please select ${Green_font_prefix} NO ${Font_color_suffix}" && echo
...
Other_functions(){
  ...
  ${Green_font_prefix}2.${Font_color_suffix} Configuration Server Speeder
  ${Green_font_prefix}3.${Font_color_suffix} Configure LotServer (Server Speeder parent company)
  ${Tip} Server Speeder/LotServer/BBR does not support OpenVZ！
  ${Tip} Server Speeder and LotServer cannot coexist！
————————————————————————————————————————————————
  ${Green_font_prefix}4.${Font_color_suffix} 一Key to block BT/PT/SPAM (iptables)
  ${Green_font_prefix}5.${Font_color_suffix} 一Key to unblock BT/PT/SPAM (iptables)
————————————————————————————————————————————————
  ${Green_font_prefix}6.${Font_color_suffix} Switch ShadowsocksR log output mode
—— Note: SSRdefault only outputs error logs, this item can be switched to output detailed access logs.
  ${Green_font_prefix}7.${Font_color_suffix} Monitor the running status of the ShadowsocksR server
—— Description: This function is suitable for the end of the process of the SSR server often. After starting this function, it will be detected every minute. When the process does not exist, the SSR server will be automatically started." && echo
...
Set_config_connect_verbose_info(){
	...
	if [[ ${connect_verbose_info} = "0" ]]; then
		...
		echo -e "Make sure to switch to ${Green_font_prefix} detailed mode (output detailed connection log + error log)${Font_color_suffix}？[y/N]"
...
Set_crontab_monitor_ssr(){
	...
	if [[ -z "${crontab_monitor_ssr_status}" ]]; then
		...
		echo -e "Are you sure you want to enable the ${Green_font_prefix}ShadowsocksR server running status monitoring ${Font_color_suffix} function? (When the process is closed, the SSR server is automatically started)[Y/n]"
		...
		echo -e "Are you sure you want to turn off the ${Font_color_suffix} function for monitoring the running status of the ${Green_font_prefix}ShadowsocksR server? (When the process is closed, the SSR server is automatically started)[y/N]"
...
crontab_monitor_ssr(){
	...
	check_pid
	if [[ -z ${PID} ]]; then
		echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] It is detected that the ShadowsocksR server is not running, starting..." | tee -a ${ssr_log_file}
		/etc/init.d/ssrmu start
		sleep 1s
		check_pid
		if [[ -z ${PID} ]]; then
...
menu_status(){
	if [[ -e ${ssr_folder} ]]; then
		check_pid
		if [[ ! -z "${PID}" ]]; then
			echo -e " Current status: ${Green_font_prefix} has installed ${Font_color_suffix} and ${Green_font_prefix} has been activated${Font_color_suffix}"
		else
			echo -e " Current status: ${Green_font_prefix} has installed ${Font_color_suffix} but ${Red_font_prefix} has not started${Font_color_suffix}"
		fi
		cd "${ssr_folder}"
	else
...
————————————————————————————————————————————————
  ...
  ${Green_font_prefix}9.${Font_color_suffix} Configuration flow reset
————————————————————————————————————————————————
...
————————————————————————————————————————————————
 ...
 ${Green_font_prefix}16.${Font_color_suffix} Quit
"
	menu_status
	echo && read -e -p "Please key in numbers [1-16]：" num
case "$num" in
	1)
	Install_SSR
	;;
	...
	;;
	*)
	echo -e "${Error} Please enter the correct number [1-16]"
	;;
esac
fi
